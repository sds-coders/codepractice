...
class Engine(object):
    ...
    def play(self):
        ...
        while True:
            print "\n--------"
            # next_scene_name = CentralCorridor() --> 무한루프
...
class CentralCorridor(Scene):
    ...
    def enter(self):
        ...
        if action == "발사!":
            ...
            print """
            세계적인 권투 선수처럼 달리고 피하고 구리고 오른쪽으로 미끄러져 
...
class TheBridge(Scene):
    ...
    def enter(self):
        ...
        if action == "폭탄 던지기":
            ...
            뛰어 오릅니다. 폭탄을 놓자마자 고던인이 뒤에서 당신을 쏴 맞추어 죽여버립니다.
            죽어가는 동안 다른 고던인이 미친듯이 폭타을 해체하려는 것을
            봅니다. 당신은 폭탄이 터지면 고던인 모두가 텨저 나가리라고 깨달으며 죽습니다.
            """
            return 'death'
        
        elif action == "천천히 폭탄 설치하기":
            print """
            팔에 낀 폭탄을 광선총으로 겨누자 고던인들은 팔을 들고 땀을 흘리기 시작합니다.
            당신은 문 뒤에 바짝 붙어 열고 광선총을 겨눈 채로 조심스럽게 폭탄을 바닥에 설치합니다.
...
class EscapePod(Scene):
    ...
    def enter(self):
        ...
        합니다. 이 중 몇 개는 손상되었을 수 있지만 살펴볼 시간이 없습니다.
        ...
        if int(guess) != good_pod:
            ...
            구명정은 우주의 진공으로 빠져나가고, 콩깍지가 터지듯 안으로 무너지며
            당신을 곤약처럼 으스러뜨립니다.            
            """ % guess
            return 'death'
...
a_map = Map('central_corridor')  # 시작장면은 central_corridor. a_map은 Map의 인스턴스
a_game = Engine(a_map)  # a_game은 Engine의 인스턴스.
